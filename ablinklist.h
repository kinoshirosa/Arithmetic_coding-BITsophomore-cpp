...
ListNode<type>* ablinklist<type>::Find_value(type value) {
	...
	int i = 1;
	while (i++ <= this->length && p->data != value)
		p = p->next;
	return p;
}
...
template <class type>
ListNode<type>& seq_find(const ablinklist<type>& list, const ListNode<type>& finding_node) {
	ListNode<type>* p = list.GetHead()->next;
	while (p != nullptr) {
		if (p->data == finding_node.data) {
			break;
		}
		p = p->next;
	}
...
template <class type>
ListNode<type>& seq_find(const ablinklist<type>& list, const type& finding_data) {
	ListNode<type>* p = list.GetHead()->next;
	while (p != nullptr) {
		if (p->data == finding_data) {
			break;
		}
		p = p->next;
	}
